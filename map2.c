 ...
 * 2D tree. Then search through the tree for the business(es) within a given
 * radius of a given point. Writes the result to a file.
 *
 * Algorithms are fun!!
 *
 */
...
void search_and_print(char *filename, treenode_t *storage_tree) {
	...
	double search_x, search_y, radius;
	
	while (scanf("%lf %lf %lf\n", &search_x, &search_y, &radius)==3) {
		// assuming that each new key to search for is on a new line
		
		// find the nodes within radius of x,y
		find_in_radius(storage_tree, search_x, search_y, radius, output_file);
	}
	fclose(output_file);
}
